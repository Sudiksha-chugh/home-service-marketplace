      ...
      <div className="bg-white rounded-3xl max-w-lg w-full h-[520px] shadow-2xl flex flex-col overflow-hidden border border-slate-200">
        ...
        <div className="bg-slate-900 text-white p-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-sm">Live Customer Chat</h3>
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-slate-50">
          {messages.length === 0 ? (
            <div className="text-center py-12 text-slate-400 text-xs">
              No messages yet. Send a message to chat with the customer live!
            </div>
          ) : (
            messages.map((msg) => {
        ...
        <form onSubmit={handleSend} className="p-3 bg-white border-t border-slate-200 flex items-center gap-2">
          <input
            ...
            className="flex-1 bg-slate-100 border border-slate-200 rounded-xl px-3.5 py-2 text-xs font-medium focus:outline-none focus:border-emerald-500"
          />
          <button
            type="submit"
